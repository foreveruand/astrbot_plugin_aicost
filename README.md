...
- 使用 AstrBot 原生 `html_render` / t2i 管线生成报告图片
...
- **`t2i_full_page`**: 是否渲染完整页面
  - `true`: 渲染完整内容页面
  - `false`: 按报告内容高度裁切，减少下半部分留白（取决于 t2i 端点是否支持 `clip` / `viewport` 选项）
...
- `t2i_image_type` - 控制输出图片格式
- `t2i_image_quality` - 控制 JPEG 输出质量
- `t2i_scale` - 控制截图缩放策略
- `t2i_full_page` - 控制是否裁切到报告内容高度
...
2. 图片报告使用 AstrBot 原生 t2i 渲染；输出尺寸能力取决于当前 t2i 端点支持的截图选项。
...
5. 若 `t2i_full_page=false` 仍出现明显留白，请确认当前 AstrBot t2i 端点支持 `clip` / `viewport` 渲染选项。
